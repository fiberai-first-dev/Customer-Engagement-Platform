...
export function ConversationThread({
  ...
  const channelRows = useMemo(
    () => Object.values(conversationsByChannel).filter(Boolean) as Conversation[],
    [conversationsByChannel],
  );
  const needsAttentionHere = Boolean(
    selectedConversation && isActiveStatus(selectedConversation.status),
  );
  const nextOpenElsewhere = useMemo(() => {
    if (!selectedConversation || needsAttentionHere) return null;
    return nextUnresolvedConversation(channelRows, selectedConversation.id);
  }, [channelRows, selectedConversation, needsAttentionHere]);
  ...
  const handleSend = () => {
    ...
    onSend(content, activeTab === "email" ? subject.trim() : undefined);
    setDraft("");
    setSubject("");
  };
    ...
    <div className="flex h-full min-w-0 flex-1 flex-col bg-background">
      <div className="flex h-14 shrink-0 items-center justify-between border-b border-border bg-card px-5">
        ...
        <div className="flex shrink-0 items-center gap-2">
          {selectedConversation && needsAttentionHere && (
            <Button
              variant="outline"
              size="sm"
              onClick={onResolve}
      ...
      {!needsAttentionHere && nextOpenElsewhere && (
        <div className="flex items-center justify-between gap-3 border-b border-border bg-muted/40 px-5 py-2 text-xs text-muted-foreground">
          <span>
            This thread is resolved.{" "}
            <span className="font-medium text-foreground">
              {channelLabel(nextOpenElsewhere.channelType)}
            </span>{" "}
            is still open.
          </span>
          <button
            type="button"
            onClick={() => onTabChange(nextOpenElsewhere.channelType)}
                  ...
                  >
                    ...
                    <div
                      className={cn(
                        ...
                        incoming ? "text-muted-foreground" : "text-primary-foreground",
                      )}
                    >
    ...
    <div className="flex h-full flex-1 flex-col items-center justify-center gap-3 bg-background px-6 text-center">
      ...
      <div>
        ...
        <p className="mt-1 max-w-sm text-sm text-muted-foreground">
          Open a contact from Unresolved — we&apos;ll jump you to the channel that still needs
          resolve.
        </p>
      </div>
    </div>
